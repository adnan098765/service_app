...
class PhoneNumberController extends GetxController {
  ...
  Future<void> verifyPhoneNumber(BuildContext context) async {
    ...
    try {
      ...
      final checkUserResponse = await http
          .post(
        ...
        if (createUserModel.success == true) {
          ...
          final otpResponse = await http
              .post(
                ...
                headers: {'Content-Type': 'application/json', 'Accept': 'application/json'},
                body: jsonEncode({'mobile': fullPhone, 'country_code': '+92', 'message_type': 'sms'}),
              )
              .timeout(
                const Duration(seconds: 10),
                onTimeout: () {
          ...
          if (otpResponse.statusCode == 200) {
            ...
            if (sendOtpModel.success == true) {
              ...
              debugPrint(
                "📲 [PhoneNumberController] Navigating to OtpScreen with phone: ${phoneNumberController.text}",
              );
              Get.to(
                () => OtpScreen(
