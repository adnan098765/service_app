...
class AllHomeServicesCategoryController extends GetxController {
  ...
  Future<void> fetchCategories() async {
    ...
    try {
      ...
      debugPrint(
          'AllHomeServicesCategoryController: Retrieved token=$token, customerId=$customerId');

      if (token == null || customerId == null) {
        debugPrint(
            'AllHomeServicesCategoryController: Authentication data missing');
      ...
      final url = Uri.parse("${AppUrls.allCategoryServices}$customerId");
      ...
      debugPrint('AllHomeServicesCategoryController: Response status=${response
          .statusCode}, body=${response.body}');

      if (response.statusCode == 200) {
        final jsonResponse = jsonDecode(response.body);
        debugPrint(
            'AllHomeServicesCategoryController: Parsed JSON response=$jsonResponse');

        final allCategoryServices = AllCategoryServices.fromJson(jsonResponse);
        debugPrint(
          'AllHomeServicesCategoryController: Parsed success=${allCategoryServices
              .success}, data=${allCategoryServices.data?.toJson()}',
        );

        if (allCategoryServices.success == true &&
            allCategoryServices.data != null) {
          if (allCategoryServices.data!.categories != null &&
              allCategoryServices.data!.categories!.isNotEmpty) {
            categories.assignAll(allCategoryServices.data!.categories!);
            debugPrint(
              'AllHomeServicesCategoryController: Added ${allCategoryServices
                  .data!.categories!.length} categories: ${categories.map((c) =>
                  c.toJson())}',
            );
          } else {
            debugPrint(
                'AllHomeServicesCategoryController: No categories found in response');
            // Only set error if no services are available either
            if (allCategoryServices.data!.services == null ||
                allCategoryServices.data!.services!.isEmpty) {
              errorMessage.value = 'No categories or services available';
            }
          }
          if (allCategoryServices.data!.services != null &&
              allCategoryServices.data!.services!.isNotEmpty) {
            services.assignAll(allCategoryServices.data!.services!);
            debugPrint(
              'AllHomeServicesCategoryController: Added ${allCategoryServices
                  .data!.services!.length} services: ${services.map((s) =>
                  s.toJson())}',
            );
          } else {
            debugPrint(
                'AllHomeServicesCategoryController: No services found in response');
            if (allCategoryServices.data!.categories == null ||
                allCategoryServices.data!.categories!.isEmpty) {
              errorMessage.value = 'No categories or services available';
            }
          }
          debugPrint(
              'AllHomeServicesCategoryController: Total services=${allCategoryServices
                  .data!.totalServices}');
        } else {
          debugPrint(
              'AllHomeServicesCategoryController: Success=false or no data');
          errorMessage.value =
              jsonResponse['message'] ?? 'No categories or services found';
          Get.snackbar(
            'Error',
            errorMessage.value,
            backgroundColor: Colors.red,
        ...
        try {
          ...
          debugPrint(
              'AllHomeServicesCategoryController: Error parsing error response: $e');
        }
      ...
      debugPrint(
          'AllHomeServicesCategoryController: Error fetching categories: $e');
