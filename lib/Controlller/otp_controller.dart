  ...
  Future<void> verifyOtpAlternative(String phoneNumber, int customerId, String? token, String? actualOtp) async {
    ...
    try {
      if (actualOtp != null && otpController.text == actualOtp) {
        ...
        debugPrint("📲 [OtpController] Local OTP verified, navigating to AuthenticationScreen with phone: $phoneNumber");
        Get.off(() => AuthenticationScreen(
          customerId: customerId,
          token: token,
          phoneNumber: phoneNumber,
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true) {
          ...
          debugPrint("📲 [OtpController] API OTP verified, navigating to AuthenticationScreen with phone: $phoneNumber");
          Get.off(() => AuthenticationScreen(
            customerId: customerId,
            token: responseData['token'] ?? token,
            phoneNumber: phoneNumber,
